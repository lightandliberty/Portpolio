...
namespace RawInput_dll
{
    ...
    internal enum RawInputDeviceFlags
    {
        ...
        INPUTSINK = 0x00000100,     // 호출 쪽에서 입력을 받는다. 심지어 호출하는 쪽이 전면에 있지 않더라도. 단 hwndTarget은 반드시 명시되어야 한다. // Enables the caller to receive the input even when the caller is not in the foreground. Note that WindowHandle must be specified.
        ...
        DEVNOTIFY = 0x00002000      // 호출하는 쪽에서 장치 도착과 장치 제거의 WM_INPUT_DEVICE_CHANGE 알림을 받는 것이 가능하도록 한다.
    }
